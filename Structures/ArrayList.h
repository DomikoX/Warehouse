...
template<typename T>
  bool equalFunction(const T& a, const T& b) {
	return a == b;
}
	...
	public:
		...
		ArrayList(const size_t initialCapacity = 2, const EqualityFunction<T>& equalityFunction = equalFunction);
		...
		void sortBy(bool(*sortFcion)(T a, T b));
		void shrinkToFit();
	...
	template<typename T>
	inline void ArrayList<T>::sortBy(bool(*sortFcion)(T a, T b))
	{
		...
		while (not_sorted) {
			...
			for (int i = 0; i < size_ - j; i++) {
				if (sortFcion((*array_)[i+1], (*array_)[i])) {
					swap(i, i + 1);
					not_sorted = true;
	...
	inline void ArrayList<T>::swap(int index1, int index2)
	{
		...
		DSRoutines::rangeCheck(index1, size_ + 1, true);
		DSRoutines::rangeCheck(index2, size_ + 1, true);

		DSRoutines::swap((*array_)[index1], (*array_)[index2]);
	...
	void ArrayList<T>::changeCapacity(size_t newCapacity)
	{
		...
		Array<T>::copy(*array_, 0 ,*nove, 0, newCapacity > (*array_).size() ? (*array_).size() : newCapacity);
		delete array_;
		array_ = nove;
